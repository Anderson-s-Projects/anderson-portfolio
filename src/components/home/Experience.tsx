  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      items.forEach((item, index) => {
        ...
        setTimelineItems(prev => prev.map((timelineItem, i) => i === index ? {
          ...timelineItem,
          isActive: isVisible
        } : timelineItem));
      });
  ...
  return <section id="experience" className="py-16 md:py-24 relative">
      <div className="section-container">
        ...
        <div ref={timelineRef} className="relative mt-16 max-w-3xl mx-auto">
          ...
          {/* Timeline items */}
          {timelineItems.map((item, index) => <div key={item.id} className={`timeline-item relative mb-16 ${index % 2 === 0 ? 'md:ml-auto md:text-right md:pr-12' : 'md:mr-auto md:pl-12'} ${item.isActive ? 'animate-fade-in opacity-100' : 'opacity-0'} transition-opacity duration-700`} style={{
          width: '85%',
          maxWidth: '500px',
          transitionDelay: `${index * 200}ms`
        }}>
              {/* Timeline connector */}
              <div className="timeline-connector mx-[61px]" />
